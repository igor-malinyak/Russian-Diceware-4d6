...
function ensureInputArtifactsExist(): void {
  if (!fs.existsSync(ARTIFACTS.dictionaryTopWithAttributes)) {
    throw new Error(
      'Missing data/attributes/dictionary-top-with-attributes.csv. Run the attributes pipeline first.',
    );
  }

  if (!fs.existsSync(ARTIFACTS.extraLemmasWithAttributes)) {
    throw new Error(
      'Missing data/attributes/extra-lemmas-with-attributes.csv. Run the attributes pipeline first.',
    );
  }
