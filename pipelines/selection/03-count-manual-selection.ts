...
function ensureInputArtifactExists(): void {
  if (!fs.existsSync(ARTIFACTS.manualSelectionCompleted)) {
    throw new Error(
      'Missing data/selection/manual-selection-completed.csv. Fill in the completed manual selection file first.',
    );
  }
}
...
console.log(
  JSON.stringify(
    {
      input: 'data/selection/manual-selection-completed.csv',
