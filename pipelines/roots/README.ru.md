...
## Методика
Этот процесс определяет корни для лемм из `data/roots/dictionary-source.csv` и приводит результат к единой итоговой системе.
...
- `data/roots/dictionary-source-with-roots.csv` — финальный словарь лемм с итоговыми корнями
- `data/roots/root-ipm.csv` — агрегированный словарь корней с суммарным `IPM` по всем леммам, где встречается корень

## Источники
- `data/roots/dictionary-source.csv` - отфильтрованная версия словаря [`data/external/freqrnc2011.csv`](../../data/external/README.ru.md#freqrnc2011csv): оставлены только существительные, глаголы, прилагательные и наречия.
- `data/roots/lemmas_to_roots_fixed.tsv` - вручную исправленная версия [`data/external/lemmas_to_roots.tsv`](../../data/external/README.ru.md#lemmas_to_rootstsv); были поправлены опечатки и OCR-ошибки, обнаруженные при сопоставлении.
- `data/roots/root_groups_fixed.txt` - вручную исправленная версия [`data/external/root_groups.txt`](../../data/external/README.ru.md#root_groupstxt); были поправлены опечатки и OCR-ошибки, обнаруженные при сопоставлении.
- `data/external/train_Tikhonov_reformat.txt` - обучающая выборка по морфемному разбиению, полученная из словаря Тихонова; см. [описание источника](../../data/external/README.ru.md#train_tikhonov_reformattxt).

## Выбор примеров
Когда артефакт корней хранит примеры лемм:
- примеры сортируются по убыванию `IPM`
- сохраняются top-10 примеров
- если лемма отсутствует в `data/roots/dictionary-source.csv`, её `IPM` считается равным `0`
...
Читает:
- `data/roots/dictionary-source.csv`
...
## Примечания
- Не изменяйте `data/roots/dictionary-source.csv`.
