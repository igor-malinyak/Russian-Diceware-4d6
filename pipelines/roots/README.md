...
## Method
This pipeline identifies roots for lemmas from `data/roots/dictionary-source.csv` and reduces the result to one consistent final root inventory.
...
- `data/roots/dictionary-source-with-roots.csv` is the final lemma dictionary with final roots
- `data/roots/root-ipm.csv` is the aggregated root dictionary with total `IPM` summed over every lemma that contains each root

## Sources
- `data/roots/dictionary-source.csv` - a filtered version of [`data/external/freqrnc2011.csv`](../../data/external/README.md#freqrnc2011csv), keeping only nouns, verbs, adjectives, and adverbs.
...
- if a lemma is missing from `data/roots/dictionary-source.csv`, its `IPM` is treated as `0`
...
## Notes
- Do not modify `data/roots/dictionary-source.csv`.
