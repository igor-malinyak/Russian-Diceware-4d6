...
function ensureInputArtifactsExist(): void {
  if (!fs.existsSync(ARTIFACTS.extraLemmasLlmAttributesOriginal)) {
    throw new Error(
      'Missing data/attributes/extra-lemmas-llm-attributes.original.csv.',
    );
  }
  if (!fs.existsSync(ARTIFACTS.extraLemmasLlmAttributesLlm)) {
    throw new Error(
      'Missing data/attributes/extra-lemmas-llm-attributes.llm.csv. Fill the LLM attributes first.',
    );
  }
  if (!fs.existsSync(ARTIFACTS.dictionarySourceWithRoots)) {
    throw new Error(
      'Missing data/roots/dictionary-source-with-roots.csv. Run the roots pipeline first.',
    );
  }
  if (!fs.existsSync(ARTIFACTS.rootIpm)) {
    throw new Error('Missing data/roots/root-ipm.csv. Run the roots pipeline first.');
  }
}
...
function loadDictionaryData(): DictionaryData {
  ...
  for (const row of dictionary.rows) {
    ...
    if (!number) {
      throw new Error('Empty Number in data/roots/dictionary-source-with-roots.csv');
    }
    if (rowsByNumber.has(number)) {
      throw new Error(
        `Duplicate Number ${number} in data/roots/dictionary-source-with-roots.csv`,
      );
    }
...
function loadRootIpmByName(): Map<string, RootIpm> {
  ...
  for (const row of rootIpm.rows) {
    ...
    if (!root) {
      throw new Error('Empty root in data/roots/root-ipm.csv');
    }
    if (rootIpmByName.has(root)) {
      throw new Error(`Duplicate root ${root} in data/roots/root-ipm.csv`);
    }
...
function buildOutputRows(): { header: string[]; rows: string[][] } {
  ...
  if (original.header.join('\u0000') !== llmAttributes.header.join('\u0000')) {
    throw new Error(
      'Header mismatch: data/attributes/extra-lemmas-llm-attributes.original.csv vs data/attributes/extra-lemmas-llm-attributes.llm.csv',
    );
  }
  ...
  for (const columnName of ATTRIBUTE_COLUMNS) {
    if (dictionary.header.includes(columnName)) {
      throw new Error(
        `Column ${columnName} already exists in data/roots/dictionary-source-with-roots.csv`,
      );
    }
  }
  ...
  const rows = original.rows.map((row, rowIndex) => {
    ...
    if (!dictionaryRow) {
      throw new Error(
        `Missing Number ${number} for lemma ${lemma} in data/roots/dictionary-source-with-roots.csv`,
      );
    }
...
console.log(
  JSON.stringify(
    {
      inputLemmas: 'data/attributes/extra-lemmas-llm-attributes.original.csv',
      inputAttributes: 'data/attributes/extra-lemmas-llm-attributes.llm.csv',
      inputDictionary: 'data/roots/dictionary-source-with-roots.csv',
      inputRootIpm: 'data/roots/root-ipm.csv',
      output: 'data/attributes/extra-lemmas-with-attributes.csv',
