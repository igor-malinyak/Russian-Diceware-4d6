...
function ensureInputArtifactsExist(): void {
  if (!fs.existsSync(ARTIFACTS.dictionaryTop)) {
    throw new Error('Missing data/attributes/dictionary-top.csv. Run step 01 first.');
  }
  if (!fs.existsSync(ARTIFACTS.llmAttributesLlm)) {
    throw new Error('Missing data/attributes/llm-attributes.llm.csv. Run step 02 first.');
  }
}
...
function buildOutputRows(): { header: string[]; rows: string[][] } {
  ...
  for (const columnName of ATTRIBUTE_COLUMNS) {
    if (dictionaryTop.header.includes(columnName)) {
      throw new Error(`Column ${columnName} already exists in data/attributes/dictionary-top.csv`);
    }
  }
...
console.log(
  JSON.stringify(
    {
      inputDictionary: 'data/attributes/dictionary-top.csv',
      inputAttributes: 'data/attributes/llm-attributes.llm.csv',
      output: 'data/attributes/dictionary-top-with-attributes.csv',
