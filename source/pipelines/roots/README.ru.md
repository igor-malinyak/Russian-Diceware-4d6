...
## Методика
Этот процесс определяет корни для лемм из `source/data/roots/dictionary-source.csv` и приводит результат к единой итоговой системе.

В основе лежат три источника морфемной информации:
- словарь Кузнецовой задаёт опорную систему корней, различение омонимов и правила нормализации
- словарь Тихонова расширяет покрытие и помогает восстановить корни там, где прямого покрытия Кузнецовой недостаточно
- LLM используется как резервный способ определения корней для лемм, которые не удаётся надёжно покрыть словарными данными

Концептуально работа устроена так:
- сначала строится опорный реестр корней Кузнецовой как основная система отсчёта
- затем тихоновские корни сопоставляются с этой системой там, где связь можно установить достаточно надёжно
- случаи, которые не удаётся уверенно разрешить автоматически, переводятся на LLM-этапы
- после разрешения всех случаев результат собирается в единый словарь лемм и в агрегированный словарь корней

Здесь важно различать два уровня представления корня:
- рабочий уровень — конкретный вариант корня, на котором удерживаются различия между омонимами и принимаются решения о сопоставлении
- итоговый уровень — каноническая форма, которая попадает в финальный словарь и используется для агрегации; для смыслово одного и того же корня с фонетическими чередованиями гласных и согласных или с выпадением гласных выбирается одна общая форма записи

Для корней, уже известных в системе Кузнецовой, каноническая форма задаётся её группировкой корней.
Для новых корней, появляющихся вне этого покрытия, каноническая форма совпадает с самим корнем.

LLM используется только в тех местах, где нужен содержательный выбор, а не механическое преобразование:
- когда тихоновский корень нельзя надёжно сопоставить с опорной системой автоматически
- когда нужно выбрать точный омонимичный вариант
- когда остаются смешанные или остаточные случаи на уровне отдельной леммы

В результате весь процесс сводит разные источники и разные типы решений к одной согласованной записи корней: в финальном словаре каждая лемма получает корни в канонической форме, независимо от того, были ли они взяты напрямую из словаря Кузнецовой, выведены автоматически из словаря Тихонова или определены на LLM-этапах.

## Артефакты
Конечные артефакты этого процесса:
- `source/data/roots/dictionary-source-with-roots.csv` — финальный словарь лемм с итоговыми корнями
- `source/data/roots/root-ipm.csv` — агрегированный словарь корней с суммарным `IPM` по всем леммам, где встречается корень

## Источники
- `source/data/roots/dictionary-source.csv` - отфильтрованная версия словаря [`source/data/external/freqrnc2011.csv`](../../data/external/README.ru.md#freqrnc2011csv): оставлены только существительные, глаголы, прилагательные и наречия.
- `source/data/roots/lemmas_to_roots_fixed.tsv` - вручную исправленная версия [`source/data/external/lemmas_to_roots.tsv`](../../data/external/README.ru.md#lemmas_to_rootstsv); были поправлены опечатки и OCR-ошибки, обнаруженные при сопоставлении.
- `source/data/roots/root_groups_fixed.txt` - вручную исправленная версия [`source/data/external/root_groups.txt`](../../data/external/README.ru.md#root_groupstxt); были поправлены опечатки и OCR-ошибки, обнаруженные при сопоставлении.
- `source/data/external/train_Tikhonov_reformat.txt` - обучающая выборка по морфемному разбиению, полученная из словаря Тихонова; см. [описание источника](../../data/external/README.ru.md#train_tikhonov_reformattxt).

## Выбор примеров
Когда артефакт корней хранит примеры лемм:
- примеры сортируются по убыванию `IPM`
- сохраняются top-10 примеров
- если лемма отсутствует в `source/data/roots/dictionary-source.csv`, её `IPM` считается равным `0`
...
- контекстный файл для LLM: `source/data/roots/kuznetsova-roots.csv`
...
- в промпте явно описан процесс разбиения на чанки и последующей сборки
- контекстный файл для LLM: `source/data/roots/resolved-roots.csv`
...
- передать ей файл `source/data/roots/llm-tikhonov-root-mapping.original.csv`
- приложить `source/data/roots/kuznetsova-roots.csv` как контекст
...
- LLM/агент должен:
  ...
  - выполнить merge
  - оставить итоговый результат в `source/data/roots/llm-tikhonov-root-mapping.llm.csv`
...
- передать ей файл `source/data/roots/llm-tikhonov-homonym-disambiguation.original.csv`
- приложить `source/data/roots/kuznetsova-roots.csv` как контекст
- также дать модели эти вспомогательные скрипты как контекст:
...
- LLM/агент должен:
  ...
  - записать соответствующие chunk `.llm.csv`
  - выполнить merge
  - оставить итоговый результат в `source/data/roots/llm-tikhonov-homonym-disambiguation.llm.csv`
...
- передать ей файл `source/data/roots/llm-roots.original.csv`
- приложить `source/data/roots/resolved-roots.csv` как контекст
- также дать модели эти вспомогательные скрипты как контекст:
...
- LLM/агент должен:
  ...
  - записать соответствующие chunk `.llm.csv`
  - выполнить merge
  - оставить итоговый результат в `source/data/roots/llm-roots.llm.csv`
...
## Примечания
- Не изменяйте `source/data/roots/dictionary-source.csv`.
